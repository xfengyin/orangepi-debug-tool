...
impl GpioService {
    pub fn new(registry: Arc<Mutex<DeviceAdapterRegistry>>, config: &GpioDeviceConfig) -> Self {
        ...
        metrics.register_histogram(
            "gpio_write_latency_ms",
            "GPIO write latency in milliseconds",
        );
    ...
    pub async fn set_direction(&self, pin: u32, direction: GpioDirection) -> AppResult<()> {
        ...
        adapter.set_direction(pin, direction).await.map_err(|e| {
            AppError::Gpio(format!("Failed to set direction for pin {}: {}", pin, e))
        })?;
    ...
    pub async fn read_pin(&self, pin: u32) -> AppResult<u8> {
        ...
        self.metrics
            .observe_histogram("gpio_read_latency_ms", elapsed);
    ...
    pub async fn write_pin(&self, pin: u32, value: u8) -> AppResult<()> {
        if value > 1 {
            return Err(AppError::InvalidArgument(
                "GPIO value must be 0 or 1".to_string(),
            ));
        }
        ...
        self.metrics
            .observe_histogram("gpio_write_latency_ms", elapsed);
    ...
    pub async fn enable_interrupt(&self, pin: u32, trigger: GpioTrigger) -> AppResult<()> {
        ...
        adapter.enable_interrupt(pin, trigger).await.map_err(|e| {
            AppError::Gpio(format!("Failed to enable interrupt on pin {}: {}", pin, e))
        })?;
        ...
        info!(
            "Enabled interrupt on GPIO pin {} with trigger {:?}",
            pin, trigger
        );
        Ok(())
    }
    ...
    pub async fn disable_interrupt(&self, pin: u32) -> AppResult<()> {
        ...
        adapter.disable_interrupt(pin).await.map_err(|e| {
            AppError::Gpio(format!("Failed to disable interrupt on pin {}: {}", pin, e))
        })?;
    ...
    pub async fn batch_read(&self, pins: &[u32]) -> AppResult<HashMap<u32, u8>> {
        ...
        let elapsed = start.elapsed().as_millis() as f64;
        self.metrics
            .add_counter("gpio_pin_reads", pins.len() as u64);
        self.metrics
            .observe_histogram("gpio_read_latency_ms", elapsed / pins.len() as f64);
    ...
    pub async fn batch_write(&self, values: &[(u32, u8)]) -> AppResult<()> {
        ...
        let elapsed = start.elapsed().as_millis() as f64;
        self.metrics
            .add_counter("gpio_pin_writes", values.len() as u64);
        self.metrics
            .observe_histogram("gpio_write_latency_ms", elapsed / values.len() as f64);
