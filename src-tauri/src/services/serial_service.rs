...
impl SerialService {
    pub fn new(registry: Arc<Mutex<DeviceAdapterRegistry>>, config: &SerialDeviceConfig) -> Self {
        ...
        metrics.register_histogram(
            "serial_read_latency_ms",
            "Serial read latency in milliseconds",
        );
    ...
    pub async fn connect(&self, port_name: &str, baud_rate: u32) -> AppResult<String> {
        if !self.config.supported_baudrates.contains(&baud_rate) {
            return Err(AppError::InvalidArgument(format!(
                ...
                baud_rate, self.config.supported_baudrates
            )));
        }
    ...
    pub async fn send(&self, connection_id: &str, data: &[u8]) -> AppResult<usize> {
        ...
        let bytes_written = adapter
            .write_internal(&connection.config, data)
            .await
            .map_err(|e| {
        ...
        self.metrics
            .observe_histogram("serial_write_latency_ms", elapsed);
        self.metrics
            .add_counter("serial_bytes_transmitted", bytes_written as u64);
    ...
    pub async fn receive(
        ...
        match result {
            Ok(Ok(bytes_read)) => {
                let elapsed = start.elapsed().as_millis() as f64;
                self.metrics
                    .observe_histogram("serial_read_latency_ms", elapsed);
                self.metrics
                    .add_counter("serial_bytes_received", bytes_read as u64);
    ...
    pub async fn set_baudrate(&self, connection_id: &str, baud_rate: u32) -> AppResult<()> {
        ...
        adapter
            .set_baudrate_internal(&connection.config, baud_rate)
            .await
