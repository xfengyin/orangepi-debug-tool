...
impl PwmService {
    ...
    pub async fn disable_channel(&self, channel: u32) -> AppResult<()> {
        ...
        adapter
            .enable_channel(0, channel, false)
            .await
    ...
    pub async fn set_frequency(&self, channel: u32, frequency_hz: u32) -> AppResult<()> {
        ...
        adapter
            .set_frequency(0, channel, frequency_hz as f64)
            .await
            .map_err(|e| {
                AppError::Pwm(format!(
                    "Failed to set frequency for channel {}: {}",
                    channel, e
                ))
            })?;
        ...
        debug!(
            "Set PWM channel {} frequency to {} Hz",
    ...
    pub async fn set_duty_cycle(&self, channel: u32, duty_percent: f64) -> AppResult<()> {
        ...
        adapter
            .set_duty_cycle(0, channel, duty_percent)
            .await
            .map_err(|e| {
                AppError::Pwm(format!(
                    "Failed to set duty cycle for channel {}: {}",
                    channel, e
                ))
            })?;
    ...
    pub async fn configure_servo(&self, channel: u32, angle: f64) -> AppResult<()> {
        ...
        let duty_cycle =
            min_duty + (angle.clamp(0.0, angle_range) / angle_range) * (max_duty - min_duty);

        self.set_duty_cycle(channel, duty_cycle).await?;
        debug!(
            "Set PWM channel {} servo angle to {} degrees (duty: {}%)",
            channel, angle, duty_cycle
        );
        Ok(())
    }
    ...
    pub async fn fade_in(&self, channel: u32, target_duty: f64, duration_ms: u64) -> AppResult<()> {
        ...
        info!(
            "Faded in PWM channel {} to {}% over {} ms",
            channel, target_duty, duration_ms
        );
        Ok(())
    }
