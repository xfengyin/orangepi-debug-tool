    ...
    fn create_pin_definitions() -> Vec<PinDefinition> {
        vec![
            PinDefinition {
                ...
                name: "PA12".to_string(),
                gpio_number: Some(12),
                modes: vec!["i2c0".to_string(), "gpio".to_string()],
            },
            PinDefinition {
                pin: 5,
                name: "PA11".to_string(),
            ...
            PinDefinition {
                pin: 8,
                name: "PG8".to_string(),
                gpio_number: Some(200),
                modes: vec!["uart2".to_string(), "gpio".to_string()],
            },
            PinDefinition {
                pin: 10,
                name: "PG9".to_string(),
                gpio_number: Some(201),
                modes: vec!["uart2".to_string(), "gpio".to_string()],
            },
            PinDefinition {
                pin: 11,
            ...
            PinDefinition {
                ...
                name: "PA14".to_string(),
                gpio_number: Some(14),
                modes: vec!["spi1".to_string(), "gpio".to_string()],
            },
            PinDefinition {
                pin: 21,
                name: "PA13".to_string(),
    ...
    async fn health_check(&self) -> AppResult<ComponentHealth> {
        ...
        #[cfg(feature = "hardware-support")]
        {
            ...
            if !std::path::Path::new(gpio_path).exists() {
                return Ok(
                    ComponentHealth::degraded(self.id(), "GPIO device not found")
        ...
        Ok(ComponentHealth::healthy(self.id()).with_latency(start.elapsed().as_millis() as u64))
    }

    async fn initialize(&self) -> AppResult<()> {
        info!(
            "Initializing OrangePi Zero3 adapter for {}",
            self.board_model
        );
